...
const loadCategoryInfo = async (): Promise<void> => {
  try {
    ...
    if (result && result.items) {
      // 查找"关于我们"分类 (slug: about_us)
      const aboutCategory = result.items.find((cat: any) => cat.slug === 'about_us')

      if (aboutCategory) {
        currentCategoryId.value = aboutCategory.id
        subCategories.value = aboutCategory.children || []
        console.log('加载到关于我们子分类:', subCategories.value.length, '个')
        // 加载分类信息后，检查是否需要根据路由设置子分类
        await setActiveSubCategoryFromRoute()
      } else {
        console.warn('未找到关于我们分类')
}
}
  } catch (error) {
...
// 监听路由变化，当从/about导航到/about/:slug时更新子分类
watch(
  () => route.params.slug,
  (newSlug, oldSlug) => {
    if (newSlug !== oldSlug && subCategories.value.length > 0) {
      setActiveSubCategoryFromRoute()
      pagination.page = 1
      loadArticles()
      // 滚动到顶部
      window.scrollTo({ top: 0, behavior: 'smooth' })
...
onMounted(async () => {
  console.log('About page mounted, route:', route.path)
