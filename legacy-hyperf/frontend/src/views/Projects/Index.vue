...
const loadCategoryInfo = async (): Promise<void> => {
  try {
    ...
    if (result && result.items) {
      // 查找"项目动态"分类 (根据实际slug调整)
      const projectCategory = result.items.find((cat: any) =>
        cat.slug === 'project_progress' ||
        cat.slug === 'projects' ||
        cat.name.includes('项目')
      )

      if (projectCategory) {
        currentCategoryId.value = projectCategory.id
        subCategories.value = projectCategory.children || []
        console.log('加载到项目动态子分类:', subCategories.value.length, '个')
        await setActiveSubCategoryFromRoute()
      } else {
        console.warn('未找到项目动态分类')
      }
    }
  } catch (error) {
...
onMounted(async () => {
  console.log('Projects page mounted, route:', route.path)
