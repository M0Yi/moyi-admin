...
const loadCategoryInfo = async (): Promise<void> => {
  try {
    ...
    if (result && result.items) {
      // 查找"信息公开"分类 (ID=21)
      const disclosureCategory = result.items.find((cat: any) => cat.id === 21)

      if (disclosureCategory) {
        currentCategoryId.value = disclosureCategory.id
        subCategories.value = disclosureCategory.children || []
        console.log('加载到信息公开子分类:', subCategories.value.length, '个')
        // 加载分类信息后，检查是否需要根据路由设置子分类
        await setActiveSubCategoryFromRoute()
      } else {
        console.warn('未找到信息公开分类')
      }
    }
  } catch (error) {
...
// 监听路由变化，当从/disclosure导航到/disclosure/:slug时更新子分类
watch(
  () => route.params.slug,
...
onMounted(async () => {
  console.log('Disclosure page mounted, route:', route.path)
