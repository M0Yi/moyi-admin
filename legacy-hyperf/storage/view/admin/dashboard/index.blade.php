...
<div class="dashboard-shell">
    ...
    @if(!empty($quickShortcuts))
    <section class="dashboard-card">
        <div class="dashboard-card__body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <p class="section-title mb-1">快捷入口</p>
                    <p class="section-subtitle mb-0">常用功能快速访问，提升工作效率</p>
                </div>
            </div>
            <div class="shortcut-grid">
                @foreach($quickShortcuts as $shortcut)
                    <a href="{{ $shortcut['url'] }}" class="shortcut-item" title="{{ $shortcut['description'] }}">
                        <div class="shortcut-item__icon" style="background: {{ $shortcut['color'] }};">
                            <i class="bi {{ $shortcut['icon'] }}"></i>
                        </div>
                        <div class="shortcut-item__label">{{ $shortcut['title'] }}</div>
                        <div class="shortcut-item__description">{{ $shortcut['description'] }}</div>
                    </a>
                @endforeach
            </div>
        </div>
