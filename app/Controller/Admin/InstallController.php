...
class InstallController extends AbstractController
{
    ...
    protected function getDefaultPermissionDefinitions(): array
    {
        return [
            ...
            [
                'name' => 'AI 智能体',
                'slug' => 'system.ai-agent',
                'type' => 'group',
                'icon' => 'robot',
                'path' => '/system/ai-agents',
                'method' => 'GET',
                'sort' => 50,
                'status' => 1,
                'children' => [
                    ...
                    [
                        'name' => '会话记录',
                        'slug' => 'system.ai-agent-sessions',
                        'type' => 'menu',
                        'icon' => 'chat-dots',
                        'path' => '/system/ai-agent-sessions*',
                        'method' => 'GET',
                        'sort' => 4,
                        'status' => 1,
                        'children' => [
                            // 查看会话记录列表
                            $this->makeButtonPermission('查看会话记录', 'system.ai-agent-sessions.view', '/system/ai-agent-sessions*', 1, 'GET'),
                            // 结束会话
                            $this->makeButtonPermission('结束会话', 'system.ai-agent-sessions.end', '/system/ai-agent-sessions/*/end', 2, 'POST'),
                            // 删除会话记录
                            $this->makeButtonPermission('删除会话记录', 'system.ai-agent-sessions.delete', '/system/ai-agent-sessions*', 3, 'DELETE'),
                        ],
                    ],
                    [
                        'name' => 'AI 日志',
                        'slug' => 'system.ai-agent-logs',
                        'type' => 'menu',
                        'icon' => 'journal-text',
                        'path' => '/system/ai-agent-logs*',
                        'method' => 'GET',
                        'sort' => 5,
                        'status' => 1,
                        'children' => [
                            // 查看交互日志列表
                            $this->makeButtonPermission('查看AI日志', 'system.ai-agent-logs.view', '/system/ai-agent-logs*', 1, 'GET'),
                            // 删除交互日志
                            $this->makeButtonPermission('删除AI日志', 'system.ai-agent-logs.delete', '/system/ai-agent-logs*', 2, 'DELETE'),
                        ],
                    ],
                ],
            ],
    ...
    protected function createDefaultMenus(int $siteId): void
    {
      ...
      AdminMenu::query()->firstOrCreate(
       ...
       AdminMenu::query()->firstOrCreate(
        ...
        // ========================================
        // AI Agent 智能体管理（位于系统管理下）
        // ========================================
        // AI 智能体管理
        AdminMenu::query()->firstOrCreate(
            ['site_id' => $siteId, 'path' => '/system/ai-agents'],
            [
                'parent_id' => $system->id,
                'name' => 'system.ai-agents',
                'title' => 'AI 智能体',
                ...
                'badge' => null,
                'badge_type' => null,
                'permission' => 'system.ai-agents.view',
                'visible' => 1,
                'status' => 1,
                'sort' => 8,
                'cache' => 1,
        ...
        // 知识库管理
        AdminMenu::query()->firstOrCreate(
            ['site_id' => $siteId, 'path' => '/system/ai-knowledge'],
            [
                'parent_id' => $system->id,
                'name' => 'system.ai-knowledge',
                'title' => '知识库',
                'icon' => 'bi bi-book',
                'component' => null,
                'redirect' => null,
                ...
                'badge' => null,
                'badge_type' => null,
                'permission' => 'system.ai-knowledge.view',
                'visible' => 1,
                'status' => 1,
                'sort' => 9,
                'cache' => 1,
        ...
        // 提供商管理
        AdminMenu::query()->firstOrCreate(
            ['site_id' => $siteId, 'path' => '/system/ai-providers'],
            [
                'parent_id' => $system->id,
                'name' => 'system.ai-providers',
                'title' => 'AI 提供商',
                'icon' => 'bi bi-cloud',
                'component' => null,
                'redirect' => null,
                ...
                'badge' => null,
                'badge_type' => null,
                'permission' => 'system.ai-providers.view',
                'visible' => 1,
                'status' => 1,
                'sort' => 10,
                'cache' => 1,
        ...
        // 会话记录
        AdminMenu::query()->firstOrCreate(
            ['site_id' => $siteId, 'path' => '/system/ai-agent-sessions'],
            [
                'parent_id' => $system->id,
                'name' => 'system.ai-agent-sessions',
                'title' => '会话记录',
                ...
                'badge' => null,
                'badge_type' => null,
                'permission' => 'system.ai-agent-sessions.view',
                'visible' => 1,
                'status' => 1,
                'sort' => 11,
                'cache' => 1,
        ...
        // 交互日志
        AdminMenu::query()->firstOrCreate(
            ['site_id' => $siteId, 'path' => '/system/ai-agent-logs'],
            [
                'parent_id' => $system->id,
                'name' => 'system.ai-agent-logs',
                'title' => 'AI 日志',
                'icon' => 'bi bi-journal-text',
                'component' => null,
                'redirect' => null,
                ...
                'badge' => null,
                'badge_type' => null,
                'permission' => 'system.ai-agent-logs.view',
                'visible' => 1,
                'status' => 1,
                'sort' => 12,
                'cache' => 1,
    ...
    private function initDatabase():void
    {
        ...
        // AI Agent 相关表
        $this->createAiAgentTables();
    }

    /**
     * 创建 AI Agent 相关数据表
     */
    private function createAiAgentTables(): void
    {
        // AI Agent 表
        if (! Schema::hasTable('ai_agents')) {
            Schema::create('ai_agents', function (Blueprint $table) {
                ...
                $table->collation = 'utf8mb4_unicode_ci';
                $table->bigIncrements('id')->comment('ID');
                $table->unsignedBigInteger('site_id')->nullable()->comment('站点ID（全局Agent为空）');
                $table->string('name', 100)->comment('Agent名称');
                $table->string('slug', 100)->unique()->comment('Agent标识（唯一）');
                $table->string('type', 50)->comment('Agent类型：audit=审核, seo=SEO, service=客服, content=内容生成, custom=自定义');
                $table->string('description', 500)->nullable()->comment('描述');
                $table->string('icon', 255)->nullable()->comment('图标');
                $table->string('class', 255)->comment('Agent类名');
                $table->json('config')->nullable()->comment('Agent配置');
                $table->tinyInteger('is_default')->default(0)->comment('是否默认Agent');
                $table->tinyInteger('status')->default(1)->comment('状态：0=禁用，1=启用');
                $table->integer('sort')->default(0)->comment('排序');
                $table->timestamp('created_at')->nullable()->comment('创建时间');
                $table->timestamp('updated_at')->nullable()->comment('更新时间');
            ...
            Db::statement("ALTER TABLE `ai_agents` COMMENT = 'AI Agent表'");
        }
        ...
        if (! Schema::hasTable('ai_agent_logs')) {
            Schema::create('ai_agent_logs', function (Blueprint $table) {
                ...
                $table->string('session_id', 64)->nullable()->comment('会话ID（客服场景）');
                $table->string('agent_name', 100)->nullable()->comment('Agent名称');
                $table->string('agent_type', 50)->nullable()->comment('Agent类型');
                $table->unsignedBigInteger('user_id')->nullable()->comment('操作用户ID');
                $table->string('username', 50)->nullable()->comment('操作用户名');
                $table->text('prompt')->nullable()->comment('输入提示词');
                $table->text('content')->nullable()->comment('待处理内容');
                $table->json('result')->nullable()->comment('处理结果');
                $table->tinyInteger('status')->default(1)->comment('状态：0=失败，1=成功');
                $table->string('error_message', 500)->nullable()->comment('错误信息');
                $table->integer('tokens')->nullable()->comment('消耗Token数');
            ...
            Db::statement("ALTER TABLE `ai_agent_logs` COMMENT = 'AI Agent使用日志表'");
        }
        ...
        if (! Schema::hasTable('ai_providers')) {
            Schema::create('ai_providers', function (Blueprint $table) {
                ...
                $table->collation = 'utf8mb4_unicode_ci';
                $table->bigIncrements('id')->comment('ID');
                $table->string('name', 100)->comment('Provider名称');
                $table->string('slug', 100)->unique()->comment('Provider标识');
                $table->string('driver', 50)->comment('驱动类');
                $table->string('base_url', 255)->comment('API基础URL');
                $table->string('api_key', 500)->nullable()->comment('API Key（加密存储）');
                $table->json('models')->nullable()->comment('可用模型列表');
            ...
            Db::statement("ALTER TABLE `ai_providers` COMMENT = 'AI Provider配置表'");
        }
        ...
        if (! Schema::hasTable('ai_agent_sessions')) {
            Schema::create('ai_agent_sessions', function (Blueprint $table) {
                ...
                $table->unsignedBigInteger('user_id')->nullable()->comment('用户ID（登录用户）');
                $table->string('user_type', 20)->default('guest')->comment('用户类型：guest=访客, member=会员, admin=管理员');
                $table->string('user_name', 50)->nullable()->comment('用户名称');
                $table->json('context')->nullable()->comment('会话上下文（历史消息）');
                $table->json('metadata')->nullable()->comment('会话元数据（来源、IP等）');
                $table->tinyInteger('status')->default(1)->comment('状态：0=结束，1=进行中');
                $table->integer('total_tokens')->default(0)->comment('累计消耗Token');
            ...
            Db::statement("ALTER TABLE `ai_agent_sessions` COMMENT = 'AI客服会话表'");
        }

        // AI 知识库表
        if (! Schema::hasTable('ai_agent_knowledge')) {
            Schema::create('ai_agent_knowledge', function (Blueprint $table) {
                $table->engine = 'InnoDB';
                $table->charset = 'utf8mb4';
                $table->collation = 'utf8mb4_unicode_ci';
                $table->bigIncrements('id')->comment('ID');
                $table->unsignedBigInteger('site_id')->nullable()->comment('站点ID');
                $table->unsignedBigInteger('agent_id')->comment('所属Agent ID');
                $table->unsignedBigInteger('category_id')->nullable()->comment('分类ID');
                $table->string('title', 255)->comment('标题');
                $table->text('content')->comment('内容');
                $table->json('keywords')->nullable()->comment('关键词（用于搜索匹配）');
                $table->tinyInteger('status')->default(1)->comment('状态：0=禁用，1=启用');
                $table->integer('sort')->default(0)->comment('排序');
                $table->timestamp('created_at')->nullable()->comment('创建时间');
                $table->timestamp('updated_at')->nullable()->comment('更新时间');
            ...
            // 添加全文索引
            try {
                Db::statement('ALTER TABLE `ai_agent_knowledge` ADD FULLTEXT INDEX `ft_title_content` (`title`, `content`)');
            } catch (\Throwable $e) {
                // 忽略索引创建失败（可能数据库不支持）
            }
            Db::statement("ALTER TABLE `ai_agent_knowledge` COMMENT = 'AI知识库表'");
        }

        // AI 知识库分类表
        if (! Schema::hasTable('ai_agent_knowledge_categories')) {
            Schema::create('ai_agent_knowledge_categories', function (Blueprint $table) {
                $table->engine = 'InnoDB';
                $table->charset = 'utf8mb4';
                $table->collation = 'utf8mb4_unicode_ci';
                $table->bigIncrements('id')->comment('ID');
            ...
            Db::statement("ALTER TABLE `ai_agent_knowledge_categories` COMMENT = 'AI知识库分类表'");
        }
        ...
        if (! Schema::hasTable('ai_agent_tools')) {
            Schema::create('ai_agent_tools', function (Blueprint $table) {
                ...
                $table->string('slug', 100)->comment('工具标识');
                $table->string('description', 500)->nullable()->comment('工具描述');
                $table->string('class', 255)->comment('工具类');
                $table->json('config')->nullable()->comment('工具配置');
                $table->tinyInteger('is_enabled')->default(1)->comment('是否启用');
                $table->integer('sort')->default(0)->comment('排序');
                $table->timestamp('created_at')->nullable()->comment('创建时间');
                $table->timestamp('updated_at')->nullable()->comment('更新时间');
                $table->unique(['slug', 'agent_id'], 'uk_slug_agent');
                $table->index('agent_id', 'idx_agent_id');
            });
            Db::statement("ALTER TABLE `ai_agent_tools` COMMENT = 'AI工具注册表'");
        }
    }
    ...
    protected function insertTestData(int $siteId, array $userIds): void
    {
        ...
        // 初始化 AI Agent 测试数据
        $this->insertAiAgentTestData();
    }

    /**
     * 初始化 AI Agent 测试数据
     */
    private function insertAiAgentTestData(): void
    {
        // 1. 插入默认 Provider（智谱 AI）
        Db::table('ai_providers')->insert([
            'name' => '智谱 AI',
            'slug' => 'zhipu',
            'driver' => 'App\Service\AiAgent\Provider\ZhipuProvider',
        ...
        Db::table('ai_agents')->insert([
            ...
            'slug' => 'audit-agent',
            'type' => 'audit',
            'description' => '用于审核文本内容的安全性，支持政治敏感、色情、暴力、广告等内容检测',
            'icon' => 'bi bi-shield-check',
            'class' => 'App\Service\AiAgent\AuditAgent',
            'config' => json_encode([
                'sensitivity' => 'medium',
                'check_items' => [
                    'politics' => true,
        ...
        Db::table('ai_agents')->insert([
            ...
            'type' => 'service',
            'description' => '智能客服 Agent，支持多轮对话、知识库问答、工具调用',
            'icon' => 'bi bi-chat-dots',
            'class' => 'App\Service\AiAgent\CustomerServiceAgent',
            'config' => json_encode([
                'system_prompt' => '你是一个专业、友好的客服助手。请根据知识库内容准确回答用户问题。如果知识库中没有相关信息，请如实告知用户，并建议用户提供更多细节。保持回答简洁、专业、有礼貌。',
        ...
        // 4. 插入示例知识库分类
        Db::table('ai_agent_knowledge_categories')->insert([
            'agent_id' => 2,
            'name' => '常见问题',
            'sort' => 0,
            'created_at' => date('Y-m-d H:i:s'),
        ...
        // 5. 插入示例知识库文档
        Db::table('ai_agent_knowledge')->insert([
            'agent_id' => 2,
            'category_id' => 1,
            'title' => '如何下单购买？',
            'content' => '您可以通过以下步骤下单：1. 浏览商品详情；2. 点击"加入购物车"；3. 填写收货地址；4. 选择支付方式完成支付。',
            'keywords' => json_encode(['下单', '购买', '支付', '购物车'], JSON_UNESCAPED_UNICODE),
            'status' => 1,
            'sort' => 0,
            'created_at' => date('Y-m-d H:i:s'),
            'updated_at' => date('Y-m-d H:i:s'),
        ...
        Db::table('ai_agent_knowledge')->insert([
            ...
            'category_id' => 1,
            'title' => '如何申请退款？',
            'content' => '申请退款步骤：1. 进入"我的订单"；2. 选择需要退款的订单；3. 点击"申请退款"；4. 填写退款原因并提交。退款将在3-5个工作日内原路返回到您的支付账户。',
            'keywords' => json_encode(['退款', '售后', '退货'], JSON_UNESCAPED_UNICODE),
            'status' => 1,
            'sort' => 1,
            'created_at' => date('Y-m-d H:i:s'),
            'updated_at' => date('Y-m-d H:i:s'),
        ...
        Db::table('ai_agent_knowledge')->insert([
            ...
            'category_id' => 2,
            'title' => '会员权益介绍',
            'content' => '我们的会员分为普通会员、银卡会员、金卡会员和黑卡会员。不同等级享有不同权益：普通会员享受基础折扣；银卡会员享受9折优惠和免费配送；金卡会员享受8.5折优惠、生日礼包和专属客服；黑卡会员享受7.5折优惠、优先发货和年度礼品。',
            'keywords' => json_encode(['会员', '权益', '等级', '折扣'], JSON_UNESCAPED_UNICODE),
            'status' => 1,
            'sort' => 0,
            'created_at' => date('Y-m-d H:i:s'),
            'updated_at' => date('Y-m-d H:i:s'),
