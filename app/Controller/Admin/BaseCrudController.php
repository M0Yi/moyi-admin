...
abstract class BaseCrudController extends AbstractController
{
     ...
     * @return ResponseInterface
     */
    public function batchToggleStatus(RequestInterface $request): ResponseInterface
    {
        try {
            ...
            if (!method_exists($service, 'batchUpdateField')) {
                return $this->error('Service 不支持批量字段更新功能', [], 500);
            }

            $count = $service->batchUpdateField($ids, $field, (int) $status);

            return $this->success(['count' => $count], "成功更新 {$count} 条记录");
        } catch (BusinessException $e) {
            return $this->error($e->getMessage(), [], $e->getCode());
        } catch (\Throwable $e) {
            return $this->error($e->getMessage());
    ...
    /**
     * 获取通用 CRUD Service 实例
     *
     * @return CrudService
     */
    protected function getCrudService(): CrudService
    {
        return make(CrudService::class);
    }
}
