 ...
 * 插件MySQL数据库管理服务
 *
 * 负责插件的数据库表创建、升级、种子数据管理等功能
 */
class AddonsMysqlService
{
    /**
     * 管理数据库表.
     *
     ...
     */
    public function manageDatabaseTables(string $addonName, string $databaseFile): bool
    {
        try {
     ...
     * 创建表并插入数据（支持升级）.
     */
    public function createTableAndInsertData(string $addonName, string $tableName, array $tableConfig): bool
    {
        try {
     ...
     * 删除表.
     */
    public function dropTable(string $addonName, string $tableName): bool
    {
        try {
     ...
     * 升级表结构.
     */
    public function upgradeTable(string $addonName, string $tableName, array $tableConfig): bool
    {
        try {
     ...
     * 获取现有表结构.
     */
    public function getExistingTableSchema(string $tableName): ?array
    {
        try {
            // 获取表字段完整信息（包含COMMENT）
     ...
     * 构建期望的表结构.
     */
    public function buildExpectedSchema(string $tableName, array $tableConfig): array
    {
     ...
     * 生成升级SQL.
     */
    public function generateAlterSqls(string $tableName, array $existing, array $expected): array
    {
     ...
     * 生成字段变更SQL.
     */
    public function generateColumnAlterSqls(string $tableName, array $existing, array $expected): array
    {
     ...
     * 生成索引变更SQL.
     */
    public function generateIndexAlterSqls(string $tableName, array $existing, array $expected): array
    {
     ...
     * 检查字段配置是否相等.
     */
    public function isColumnConfigEqual(array $existing, array $expected): bool
    {
     ...
     * 标准化字段配置，确保数据类型一致.
     */
    public function normalizeColumnConfig(array $config): array
    {
        return [
            'name' => $config['name'] ?? '',
     ...
     * 标准化字段类型.
     */
    public function normalizeType(string $type): array
    {
        // 转换为大写并标准化常见类型
     ...
     * 插入种子数据.
     */
    public function insertSeedData(string $tableName, array $data): void
    {
        foreach ($data as $row) {
            try {
                // 构建重复检查条件
     ...
     */
    public function upgradeSeedData(string $tableName, array $data): void
    {
        foreach ($data as $row) {
            try {
                // 只处理有ID字段的记录
     ...
     */
    public function buildDuplicateCheckConditions(array $row): array
    {
     ...
     * 构建创建表SQL.
     */
    public function buildCreateTableSql(string $table, array $tableConfig): string
    {
        $columns = $tableConfig['columns'] ?? [];
     ...
     * 转换列配置格式.
     */
    public function convertColumnsConfig(array $columns): array
    {
        $converted = [];
        foreach ($columns as $name => $config) {
            $column = [
     ...
     * 构建索引SQL.
     */
    public function buildIndexSqls(string $table, array $columns): array
    {
        $indexSqls = [];
     ...
     * 解析列类型.
     */
    public function parseColumnType(string $type): array
    {
        $result = [
            'length' => null,
            'unsigned' => false,
     ...
     */
    public function buildColumnDefinition(array $column, bool $includePrimaryKey = true): string
    {
        $name = $column['name'];
        $type = $column['type'];
