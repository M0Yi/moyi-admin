...
function renderFieldsConfig(columns) {
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    ${(() => {
                                        ...
                                        return `
                                            <select class="form-select form-select-sm field-col-select"
                                                   name="fields_config[${index}][col]"
                                                   data-index="${index}">
                                                <option value="" ${selectedValue === '' ? 'selected' : ''}>默认（自动）</option>
                                                <option value="col-12" ${selectedValue === 'col-12' ? 'selected' : ''}>全宽（col-12）</option>
                                                <option value="col-12 col-md-6" ${selectedValue === 'col-12 col-md-6' ? 'selected' : ''}>半宽（col-12 col-md-6）</option>
                                                <option value="col-12 col-md-4" ${selectedValue === 'col-12 col-md-4' ? 'selected' : ''}>1/3宽（col-12 col-md-4）</option>
                                                <option value="col-12 col-md-3" ${selectedValue === 'col-12 col-md-3' ? 'selected' : ''}>1/4宽（col-12 col-md-3）</option>
                                                <option value="col-12 col-md-8" ${selectedValue === 'col-12 col-md-8' ? 'selected' : ''}>2/3宽（col-12 col-md-8）</option>
                                                <option value="col-12 col-md-9" ${selectedValue === 'col-12 col-md-9' ? 'selected' : ''}>3/4宽（col-12 col-md-9）</option>
                                                <option value="col-6 col-md-2" ${selectedValue === 'col-6 col-md-2' ? 'selected' : ''}>1/6宽（col-6 col-md-2）</option>
                                                <option value="custom" ${isCustom ? 'selected' : ''}>自定义...</option>
                                            </select>
                                            <input type="text" 
                                                   class="form-control form-control-sm mt-2 field-col-custom" 
                                                   name="fields_config[${index}][col_custom]"
                                                   value="${escapeHtml(isCustom ? colValue : '')}"
                                                   placeholder="例如：col-12 col-lg-4"
                                                   style="display: ${isCustom ? 'block' : 'none'};">
                                        `;
                                    })()}
                                    <small class="text-muted">控制字段在表单中的宽度，留空使用默认值（col-12 col-md-3）</small>
                                </div>
    ...
    // 为所有列宽选择框添加事件监听
    document.querySelectorAll('.field-col-select').forEach(select => {
        select.addEventListener('change', function() {
            const index = parseInt(this.getAttribute('data-index'));
            const customInput = document.querySelector(`input[name="fields_config[${index}][col_custom]"]`);
...
function saveConfig() {
        ...
        if (key.startsWith('fields_config[')) {
            ...
            if (parsed && parsed.fieldIndex !== undefined) {
                ...
                if (parsed.parts && parsed.parts.length > 0) {
                    ...
                    if (firstPart === 'options' && secondPart !== undefined && thirdPart !== undefined) {
                    ...
                    } else if (firstPart === 'col_custom') {
                        // 处理自定义列宽：如果选择了自定义，使用自定义输入框的值
                        const colSelect = form.querySelector(`select[name="fields_config[${fieldIndex}][col]"]`);
                        if (colSelect && colSelect.value === 'custom') {
                            // 如果选择了自定义，将自定义值保存到 col 字段
                            if (!fieldsConfig[fieldIndex].col) {
                                fieldsConfig[fieldIndex].col = '';
                            }
                            // 注意：这里先保存到临时字段，稍后处理
                            if (!fieldsConfig[fieldIndex]._col_custom) {
                                fieldsConfig[fieldIndex]._col_custom = value;
                            }
                        }
                        // 不保存 col_custom 字段本身
                    } else if (firstPart) {
                        ...
                        } else if (firstPart === 'col') {
                            // 处理列宽：如果选择了自定义，使用自定义输入框的值；否则使用下拉框的值
                            const colSelect = form.querySelector(`select[name="fields_config[${fieldIndex}][col]"]`);
                            if (colSelect && colSelect.value === 'custom') {
                                // 如果选择了自定义，使用自定义输入框的值
                                const customInput = form.querySelector(`input[name="fields_config[${fieldIndex}][col_custom]"]`);
                                if (customInput && customInput.value.trim()) {
                                    fieldsConfig[fieldIndex].col = customInput.value.trim();
                                } else {
                                    // 如果自定义输入框为空，不保存 col 字段
                                    delete fieldsConfig[fieldIndex].col;
                                }
                            } else if (value && value.trim()) {
                                // 如果选择了预设值，使用下拉框的值
                                fieldsConfig[fieldIndex].col = value.trim();
                            } else {
                                // 如果为空，不保存 col 字段
                                delete fieldsConfig[fieldIndex].col;
                            }
    ...
    function cleanFieldConfig(field) {
        // 处理 col 字段：如果为空字符串，删除该字段
        if (field.col !== undefined && (!field.col || field.col.trim() === '')) {
            delete field.col;
        }
        // 删除临时字段
        if (field._col_custom !== undefined) {
            delete field._col_custom;
        }
