...
<div class="form-container">
    <form id="postForm" action="/admin/simple_blog/{{ $post->id }}" method="POST">
        ...
        <div class="row">
            ...
            <div class="col-md-8">
                <div class="mb-3">
                    ...
                    <input type="text" name="title" class="form-control" placeholder="请输入文章标题" value="{{ $post->title }}" required>
                </div>

                <div class="mb-3">
                    <label class="form-label required">文章内容</label>
                    <textarea name="content" id="contentEditor" class="form-control" rows="15" placeholder="请输入文章内容" required>{{ $post->content }}</textarea>
                </div>
            </div>
            ...
            <div class="col-md-4">
                ...
                <div class="mb-3">
                    ...
                    <div class="tag-input-wrapper">
                        ...
                        <input type="hidden" name="tags" id="tagsInput" value="{{ json_encode($post->tags ?? []) }}">
                    </div>
                ...
                <div class="mb-3">
                    <label class="form-label">
                        <input type="checkbox" name="is_featured" value="1" {{ $post->is_featured ? 'checked' : '' }}> 精选文章
                    </label>
                </div>

                <div class="mb-3">
                    <label class="form-label">
                        <input type="checkbox" name="is_pinned" value="1" {{ $post->is_pinned ? 'checked' : '' }}> 置顶文章
                    </label>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('postForm').addEventListener('submit', function(e) {
        ...
        fetch('/admin/simple_blog/{{ $post->id }}', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.code === 200) {
                ...
                alert(data.message || data.msg || '更新失败');
            }
        })
        .catch(error => {
